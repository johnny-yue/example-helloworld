...
pub fn connect(config: &Config) -> Result<RpcClient> {
    msg!("connecting to solana node at {}", config.json_rpc_url);
    let client =
...
static DEPLOY_PATH: &str = "dist/program";
...
pub fn get_program_keypair(client: &RpcClient) -> Result<Keypair> {
    ...
    msg!("loading program keypair from {}", program_keypair_path);
...
pub fn get_program_instance_account(
    ...
    if account.is_err() {
        ...
        msg!("contract size: {}", contract_size);
